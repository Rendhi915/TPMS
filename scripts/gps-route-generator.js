...
class GPSRouteGenerator {
  ...
  _initialize() {
    ...
    try {
      this.validArea = turf.buffer(this.polygon, -this.config.boundaryMargin * 111, { units: 'kilometers' });
    } catch (error) {
  ...
  getPatternHeading(currentLat, currentLng, currentHeading, pointIndex) {
    switch (this.config.routePattern) {
      case 'circular':
        ...
        const bearingToCentroid = this.calculateBearing(currentLat, currentLng, centroidLat, centroidLng);
  ...
  generateRoute(startTime = new Date()) {
    ...
    if (this.config.startLat && this.config.startLng) {
      ...
      if (this.isPointInside(this.config.startLat, this.config.startLng)) {
        ...
        console.log(`   📍 Starting from last known position: (${currentPos.lat.toFixed(6)}, ${currentPos.lng.toFixed(6)})`);
      } else {
      ...
      currentPos = this.findRandomPointInside();
      console.log(`   📍 Starting from random position: (${currentPos.lat.toFixed(6)}, ${currentPos.lng.toFixed(6)})`);
    }
    ...
    console.log(`   🏁 End: (${route[route.length - 1].lat.toFixed(6)}, ${route[route.length - 1].lng.toFixed(6)})`);
