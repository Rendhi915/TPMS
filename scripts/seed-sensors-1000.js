...
async function seedHubTempEvent(truckId) {
  ...
  const ts = new Date(Date.now() - randomInt(0, 60) * 60000);
  await prisma.hub_temperature_event
    .create({
...
async function main() {
  try {
    ...
    for (const truck of trucks) {
      ...
      await Promise.all([seedFuelEvent(truck.id), seedHubTempEvent(truck.id)]);
      processed++;
      if (processed % 100 === 0)
        console.log(`Seeded sensors for ${processed}/${trucks.length} trucks...`);
    }
