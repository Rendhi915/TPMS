...
class RealtimeTrackingSimulator {
  ...
  async startSimulation(intervalSeconds = 15) {
    ...
    console.log(
      `📊 Tracking ${this.trucks.length} trucks along ${routeCoordinates.length} route points`
    );
      ...
      try {
        ...
        if (updates.length > 0) {
          ...
          updates.forEach((update) => {
            console.log(
              `   ${update.plateNumber}: [${update.position[0].toFixed(6)}, ${update.position[1].toFixed(6)}] - ${update.speed} km/h, ${update.heading}°`
            );
          });
        }
      } catch (error) {
